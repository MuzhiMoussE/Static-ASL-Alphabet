- **Train new models or use the existing models**: 
Run the script without the --use_model argument or simply run the `main.py`:
```
python main.py
```
- **Use Existing Model**: Specify the model to use with --use_model:
```
...
- **Predict New Data**: To predict new data, use the `--predict` argument:
```
...
To generate and save the SVM performance plot with different `C` values, use the following command:
```
python main.py --plot_svm
```
The plot will be saved in the `results` directory as `svm_performance.png`.

To generate and save the KNN performance plot with different `n_neighbors` values, use the following command:
```
python main.py --plot_knn
```
The plot will be saved in the `results` directory as `knn_performance.png`.

To generate and save the Naive Bayes performance plot, use the following command:
```
python main.py --plot_naive_bayes
```
To generate and save the cross-validation performance plot, use the following command:
```
python main.py --run_cv
```
To generate and save the learning curve plot, use the following command:
```
python main.py --run_learning_curve
```
To generate and save the confusion matrix plot, use the following command:
```
